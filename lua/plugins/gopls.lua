...
--
-- Build tags are intentionally NOT set here (tag-neutral, matching main):
-- they are per-developer / per-repo, so add the tags your codebase needs in
-- the gitignored `lua/custom/plugins/gopls.lua` — see main's gopls.lua stub
-- for the override pattern (function form; the `/go-test-env` skill can
-- scaffold it via the `lsp` target).
